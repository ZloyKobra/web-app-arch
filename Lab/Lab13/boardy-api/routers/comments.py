...
async def get_comments(post_id: int):
    ...
    async with conn.cursor(aiomysql.DictCursor) as cur:
        await cur.execute(
            ...
            'WHERE c.author_id = %s ' # ← фильтр по родителю
...
async def create_post(post_id: int, data: CommentCreate):
...
async def update_post(comment_id: int, data: CommentUpdate):
...
async def delete_post(comment_id: int):
    conn = await get_db()
    async with conn.cursor() as cur:
